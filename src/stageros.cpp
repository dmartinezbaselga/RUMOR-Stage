...
void StageNode::initializeOrca(){
  if (orca_robots > 0){
    ...
    sim->setAgentDefaults(1.5f, 4.0f, 2.0f, 2.0f, 0.25f, 0.7f);
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname )
{
    ...
    if(!localn.getParam("publish_info", publish_info))
        publish_info = true;
    ...
    if (publish_info){
        ground_truth_orca_pub = n_.advertise<obstacle_detector::Obstacles>("/robot_0/obstacles", 10);
    }
...
bool StageNode::teleport_service(stage_ros_dovs::teleport::Request& request, stage_ros_dovs::teleport::Response& response){
    ...
    if (orca_robots > 0){
      ...
      for (int i = 0; i<orca_robots; i++){
            ...
            this->positionmodels[extra_robots+i]->SetSpeed(Stg::Velocity(0,0,0,0));
      }
    }
  ...
  if (orca_robots > 0 && std::string(mod->Token()).substr(0,4) == "orca"){
    ...
    if (orca_counter == 0){
      ...
      for (int i = 0; i<orca_robots; i++){
        ...
        sim->setAgentVelocity(i, RVO::Vector2(0.0,0.0));
      }
      sim->doStep();
      obstacle_detector::Obstacles obs_msg;
      ...
      for (int i = 0; i<orca_robots; i++){
        ...
        if (publish_info){
          obstacle_detector::CircleObstacle obs;
          obs.velocity.x = vel.x();
          obs.velocity.y = vel.y();
          obs.linear_vel = sqrt(vel.x()*vel.x() + vel.y()*vel.y());
          obs.radius = 0.2;
          obs.true_radius = 0.2;
          obs.theta = atan2(vel.y(), vel.x()) + this->positionmodels[extra_robots+i]->GetPose().a;
          obs.center.x = pos.x();
          obs.center.y = pos.y();
          obs_msg.circles.push_back(obs);
        }
      }
      if (publish_info){
        ground_truth_orca_pub.publish(obs_msg);
      }
    }
